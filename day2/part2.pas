...
var
  ...
  dummy,compteur,cpt,safe:integer;
  news,s,st:string;
  okwithdamper:Boolean;
...
begin
  ...
  begin 
    ...
    if isSafe(s) then inc(safe) else
    begin
      okwithdamper:=false;
      for dummy:=1 to cpt do
      begin
        news:='';
        for compteur:=1 to cpt do if (compteur<>dummy) then news:=news+inttostr(tab[compteur])+' ';
        if isSafe(TrimRight(news)) then 
        begin
          okwithdamper:=true;
          Break;
        end;
      end;
      if okwithdamper=True then inc(safe);
    end;
