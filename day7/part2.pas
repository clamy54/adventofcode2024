...
Function ContainOperateur(S:String):boolean;
var a: integer;
begin
  ContainOperateur:=false;
  for a:=1 to length(S) Do if Pos(S[a],operateur)>0 then ContainOperateur:=true;
end;
...
begin
  resultat:=0;
  if ContainOperateur(S)=true then
  begin
    trigger:=false;
    ...
    begin
      ...
      begin
        ...
        case S[dummy] of 
          ...
          '|': resultat:= StrToInt64(inttostr(resultat)+inttostr(numRight(S,dummy)));
        end;
      end;
    end;
  end
  else resultat:=StrToInt64(S);
  evalue:=resultat;
end;
...
begin
  ...
  begin
     // Writeln(S+'  : '+ inttostr(evalue(S))+' vs '+inttostr(res));
